    ...
    try:
        for i, (images, gts, grads) in enumerate(train_loader, start=1):
            ...
            if i % 20 == 0 or i == total_step or i == 1:
                ...
                writer.add_image(
                    "Pred_final", torch.tensor(res), step, dataformats="HW"
                )
                res = preds[1][0].clone()
                res = res.sigmoid().data.cpu().numpy().squeeze()
                res = (res - res.min()) / (res.max() - res.min() + 1e-8)
...
def val(test_loader, model, epoch, save_path, writer):
    ...
    with torch.no_grad():
        for _ in range(test_loader.size):
            ...
            res = F.upsample(
                res[0], size=gt.shape, mode="bilinear", align_corners=False
            )
            res = res.sigmoid().data.cpu().numpy().squeeze()
            res = (res - res.min()) / (res.max() - res.min() + 1e-8)
        ...
        if epoch == 1:
            ...
            print(
                "[Cur Epoch: {}] Metrics (mxFm={}, Sm={}, mxEm={})".format(
                    epoch,
                    metrics_dict["mxFm"],
                    metrics_dict["Sm"],
                    metrics_dict["mxEm"],
            ...
            if cur_score > best_score:
                ...
                print(
                    ">>> save state_dict successfully! best epoch is {}.".format(epoch)
                )
            else:
            ...
            print(
                "[Cur Epoch: {}] Metrics (mxFm={}, Sm={}, mxEm={})\n[Best Epoch: {}] Metrics (mxFm={}, Sm={}, mxEm={})".format(
                    ...
                    metrics_dict["mxEm"],
                    best_epoch,
                    best_metric_dict["mxFm"],
                    best_metric_dict["Sm"],
                    best_metric_dict["mxEm"],
                )
            )
            logging.info(
                "[Cur Epoch: {}] Metrics (mxFm={}, Sm={}, mxEm={})\n[Best Epoch:{}] Metrics (mxFm={}, Sm={}, mxEm={})".format(
...
def get_parser():
    ...
    parser.add_argument(
        "--save_path",
        type=str,
        default="./lib_pytorch/snapshot/Exp02/",
...
def main():
    ...
    for epoch in range(1, opt.epoch):
        ...
        writer.add_scalar(
            "learning_rate", cosine_schedule.get_lr()[0], global_step=epoch
        )
