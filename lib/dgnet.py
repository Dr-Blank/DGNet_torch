...
class SoftGroupingStrategy(nn.Module):
    def __init__(self, in_channel, out_channel, N):
        ...
        self.g_conv1 = nn.Conv2d(
            in_channel, out_channel, kernel_size=1, groups=N[0], bias=False
        )
        self.g_conv2 = nn.Conv2d(
            in_channel, out_channel, kernel_size=1, groups=N[1], bias=False
        )
        self.g_conv3 = nn.Conv2d(
            in_channel, out_channel, kernel_size=1, groups=N[2], bias=False
        )
...
class GradientInducedTransition(nn.Module):
    def __init__(self, channel, M, N):
        ...
        self.downsample2 = nn.Upsample(
            scale_factor=1 / 2, mode="bilinear", align_corners=True
        )
        self.downsample4 = nn.Upsample(
            scale_factor=1 / 4, mode="bilinear", align_corners=True
        )
    ...
    def forward(self, xr3, xr4, xr5, xg):
        ...
        q4 = self.gradient_induced_feature_grouping(
            xr4, self.downsample2(xg), M=self.M[1]
        )
        q5 = self.gradient_induced_feature_grouping(
            xr5, self.downsample4(xg), M=self.M[2]
        )
    ...
    def gradient_induced_feature_grouping(self, xr, xg, M):
        if not M in [1, 2, 4, 8, 16, 32]:
            raise ValueError("M must be in [1, 2, 4, 8, 16, 32]")
...
class NeighborConnectionDecoder(nn.Module):
    ...
    def forward(self, zt5, zt4, zt3):
        ...
        zt3_1 = (
            self.conv_upsample2(self.upsample(zt4_1))
            * self.conv_upsample3(self.upsample(zt4))
            * zt3
        )
    ...
    def __init__(self, channel=32, arc="B0", M=None, N=None):
        ...
        match arc:
            ...
            case "PVTv2-B4":
                ...
                in_channel_list = [64, 160, 256]
            case _:
                raise Exception(f"Invalid Architecture Symbol: {arc}")

        self.texture_encoder = TextureEncoder()

        self.dr3 = DimensionalReduction(
            in_channel=in_channel_list[0], out_channel=channel
        )
        self.dr4 = DimensionalReduction(
            in_channel=in_channel_list[1], out_channel=channel
        )
        self.dr5 = DimensionalReduction(
            in_channel=in_channel_list[2], out_channel=channel
        )
    ...
    def get_params(model_name: str):
        ...
        match model_name:
            ...
            case "DGNet":
                pass
            case foo:
                raise Exception(f"Invalid Model Symbol: {foo}")
