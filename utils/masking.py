...
def overlay_mask(
    ...
    for i in range(3):
        img[:, :, i] = img[:, :, i] * mask * mask_alpha + img[:, :, i] * (
            1 - mask_alpha
        )
...
def main():
    ...
    predicted_masks = Path("./result/Exp-DGNet/MOCA")
    true_images = Path("./dataset/TestDataset/MOCA")
    ...
    for folder in predicted_masks.iterdir():
        ...
        for mask in folder.iterdir():
            ...
            img_path = true_images / folder.name / "Imgs" / mask.name
